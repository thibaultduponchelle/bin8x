...
int main(int argc, char *argv[])
{
    ...
    if(cmdline->verbose) {
        puts("Bin8x v2.1 Ti-82/83/83+ squisher");
    }
...
void help(char *name, int ret)
{
    fprintf(stdout, "Bin8x v1.4 Ti-82/83/83+ squisher\n\n"
        ...
        "\t\033[01mbinfile\033[0m is the binary output from your assembler\n\n"
        "\033[01mOptions:\033[0m\n"
        "\t-i   or -input        specify inputfile, use it with -o (and possibly -n)\n"
        "\t-o   or -output       specify outputfile ([binfile].8xp if none given)\n"
        "\t-n   or -calcname     specify filename on calculator ([BINFILE] if none given)\n"
        ...
        "\t-u   or -unprotected  generate unprotected files\n"
        "\t-q   or -unsquish     unsquish the binary (useful for regular TI-83 no shell)\n"
        "\t-v   or -verbose      provide a lot of informations while running\n\n"
        "You can only specify one calculator (with -ti82 / -ti83 / -ti83p) at a time.\n\n"
